...
if __name__ == '__main__':
    ...
    # Load checkpoint
    model.load_state_dict(torch.load("/home/fei/Documents/Dataset/icra25_align_human_robot/labeled/robot/cam_104122061850/pick/weights_004/run1_grids_size21/model_epoch_200.pt"))
    model.eval()
    ...
    df.to_csv("/home/fei/Documents/Dataset/icra25_align_human_robot/labeled/robot/cam_104122061850/pick/weights_004/run1_grids_size21/result_validation_all.csv", index=False)

    # Convert to NumPy and average across samples
    avg_probs = np.mean(np.array(all_probs), axis=0).reshape(1, -1)  # shape [1, num_classes]
    ...
    ax = sns.heatmap(
        ...
        fmt=".2g",
        cmap="YlGnBu",
        xticklabels=["nothing", "reaching", "grasping", "lifting", "transporting", "holding", "placing", "releasing"],
        yticklabels=["Avg Prob"],
        cbar=True
    )

    plt.xticks(rotation=45, ha="right")
    plt.title("Average Softmax Probability per Class")
    plt.tight_layout()
    plt.savefig("avg_softmax_prob_heatmap.png", dpi=300)
    plt.show()
